...
import { useAddress, useMetamask, useDisconnect, useNFTDrop, useContract } from '@thirdweb-dev/react'
...
const Home = () => {
    ...
    const nftDrop = useContract(process.env.NEXT_PUBLIC_NFT_DROP_ADDRESS, "nft-drop")
    ...
    useEffect(() => {
        ...
        const checkAllowlist = async () => {
            ...
            try {
                const inAllowlist = allowlist.check(address)
    ...
    const joinAllowlist = async () => {
        ...
        try {
            const success = allowlist.join(address)
